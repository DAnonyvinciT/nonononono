...
def trainer(args, train_loader, dev_loader, model, optimizer, criterion, epoch=100):
    ...
    for e in range(epoch+1):
        ...
        for i, (audio, vertice, template, one_hot, file_name) in pbar:
            ...
            # 跳过非有限 loss，避免参数被污染后全程 NaN。
            if args.skip_non_finite and (not torch.isfinite(loss)):
                print("[WARN] Skip non-finite train loss at epoch {}, iter {}, file {}".format(e+1, iteration, file_name[0]))
                optimizer.zero_grad()
                continue
            ...
            if i % args.gradient_accumulation_steps==0:
                if args.skip_non_finite and has_non_finite_grad(model):
                    print("[WARN] Skip optimizer step due to non-finite gradients at epoch {}, iter {}, file {}".format(e+1, iteration, file_name[0]))
                    optimizer.zero_grad()
                    continue
                # 最小稳态保护：限制梯度范数，降低梯度爆炸概率。
                if args.max_grad_norm > 0:
                    torch.nn.utils.clip_grad_norm_(model.parameters(), args.max_grad_norm)
                optimizer.step()
                optimizer.zero_grad()

            train_loss_display = np.mean(loss_log) if len(loss_log) > 0 else float("inf")
            pbar.set_description("(Epoch {}, iteration {}) TRAIN LOSS:{:.7f}".format((e+1), iteration ,train_loss_display))
        # validation
        valid_loss_log = []
        model.eval()
        ...
        current_loss = np.mean(valid_loss_log) if len(valid_loss_log) > 0 else float("inf")
...
def main():
    ...
    parser.add_argument("--skip_non_finite", type=int, default=1, help='是否跳过非有限 loss（1:是, 0:否）')
    parser.add_argument("--max_grad_norm", type=float, default=1.0, help='梯度裁剪阈值，<=0 表示关闭')
    parser.add_argument("--disable_spec_augment", type=int, default=1, help='是否关闭 wav2vec2 的 spec augment（1:关闭, 0:开启）')
    ...
    if args.disable_spec_augment:
        # 在新版本 torch/transformers 下先关闭该增强以提升数值稳定性。
        model.audio_encoder.config.apply_spec_augment = False
