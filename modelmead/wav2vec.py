...
class Wav2Vec2Model(Wav2Vec2Model):
    def __init__(self, config):
        ...
        self._masked_spec_embed_checked = False

    def _ensure_finite_masked_spec_embed(self):
        """确保 SpecAugment 的掩码向量是有限值，避免训练首步出现 NaN。"""
        if self._masked_spec_embed_checked:
            return
        self._masked_spec_embed_checked = True

        if not hasattr(self, "masked_spec_embed"):
            return

        if torch.isfinite(self.masked_spec_embed).all():
            return

        with torch.no_grad():
            nn.init.normal_(self.masked_spec_embed, mean=0.0, std=self.config.initializer_range)
            if not torch.isfinite(self.masked_spec_embed).all():
                self.masked_spec_embed.data.zero_()
    ...
    def forward(
        ...
        # Handle both tensor and tuple output (some checkpoints return tuple)
        if isinstance(hidden_states, tuple):
            hidden_states = hidden_states[0]

        if self.config.apply_spec_augment and self.training:
            self._ensure_finite_masked_spec_embed()
