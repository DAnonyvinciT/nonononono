    ...
    for r, ds, fs in os.walk(audio_path):
        for f in tqdm(fs):
            if f.endswith("wav"):
                ...
                # 清洗音频特征中的非有限值，防止训练阶段出现 NaN。
                input_values = np.nan_to_num(input_values, nan=0.0, posinf=0.0, neginf=0.0)
                key = f.replace("wav", "npy")
                data[key]["audio"] = input_values
                subject_id = "_".join(key.split("_")[:-1])
                temp = templates[subject_id]
                data[key]["name"] = f
                data[key]["template"] = np.nan_to_num(temp.reshape((-1)), nan=0.0, posinf=0.0, neginf=0.0)
                vertice_path = os.path.join(vertices_path,f.replace("wav", "npy"))
                if not os.path.exists(vertice_path):
                    del data[key]
                else:
                    if args.dataset == "vocaset":
                        vertice = np.load(vertice_path,allow_pickle=True)[::2,:]#due to the memory limit
                        data[key]["vertice"] = np.nan_to_num(vertice, nan=0.0, posinf=0.0, neginf=0.0)
                    elif args.dataset == "BIWI":
                        vertice = np.load(vertice_path,allow_pickle=True)
                        data[key]["vertice"] = np.nan_to_num(vertice, nan=0.0, posinf=0.0, neginf=0.0)
